...
class Supabase(SupabaseInterface):
    ...
    def get_tracks_by_user_id(self, user_id: str) -> List[Track]:
        response = (
            self.client.table("tracks").select("*").eq("user_id", user_id).execute()
        )
    ...
    def get_modules_by_track_id(self, track_id: int) -> List[Module]:
        response = (
            self.client.table("modules").select("*").eq("track_id", track_id).execute()
        )
        return [Module(**item) for item in response.data]

    def get_module_by_id(self, module_id: int) -> Optional[Module]:
        response = (
            self.client.table("modules").select("*").eq("id", module_id).execute()
        )
        if response.data:
    ...
    def get_videos_by_module_id(self, module_id: int) -> List[Video]:
        response = (
            self.client.table("videos").select("*").eq("module_id", module_id).execute()
        )
    ...
    def get_activity_by_id(self, activity_id: int) -> Optional[Activity]:
        response = (
            self.client.table("activities").select("*").eq("id", activity_id).execute()
        )
        if response.data:
